...
#define NUM_POS_BITS   16                /* 64KB window, 2/2/2022 */
...
int main( int argc, char *argv[] )
{
	...
	for ( i = 0; i < WIN_BUFSIZE; i++ ) {
		lznext[i] = LZ_NULL;
		lzprev[i] = LZ_NULL;
		insert_lznode( hash_w(i) & WIN_MASK, i );
	}
...
void put_codes( dpos_t *dpos )
{
	...
	*/
	...
	for ( i = 0; i < (dpos->len+2); i++, j++ ) {
		delete_lznode( hash_w(j) & WIN_MASK, j & WIN_MASK );
	}
	...
	j = k;
	for ( i = 0; i < (dpos->len+2); i++, j++ ) {
		insert_lznode( hash_w(j) & WIN_MASK, j & WIN_MASK );
	}
